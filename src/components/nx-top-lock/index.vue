<template>
  <span>
    <el-tooltip effect="dark" content="锁屏" placement="bottom">
      <i class="icon-bofangqi-suoping" @click="handleLock" />
    </el-tooltip>
  ...
  methods: {
    handleSetLock() {
      this.$refs['form'].validate(valid => {
        if (valid) {
          this.$store.commit('user/SET_LOCK_PASSWD', this.form.passwd)
          this.handleLock()
        }
      })
    },
...
::v-deep .el-menu-item .icon-bofangqi-suoping {
    margin-right: 5px;
    width: 24px;
    text-align: center;
    font-size: 18px;
...
::v-deep .el-submenu .icon-bofangqi-suoping {
    vertical-align: middle;
    margin-right: 5px;
    width: 24px;
    text-align: center;
